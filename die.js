...
var dieunah = new function() {
	...
	this.bloquear_pantalla=function(){
		$.blockUI({ css: { 
                    border: 'none', 
                    padding: '15px', 
                    backgroundColor: '#000', 
	...
	//parent.urlRoot=preSubURL.substring(0,preSubURL.length-9);
        $.get(parent.urlRoot+"/course/view.php?id="+parent.idCurso+"&parametroDEGT=quitar",function(data,status){
					...
					resultado=data;
					/*Participantes */parent.lista.push({ id:'u0t0a0_participantes',modID:0, titulo: 'Participantes', url:parent.urlRoot+"/user/index.php?id="+parent.idCurso});
					/*Calificaciones*/parent.lista.push({ id:'u0t0a0_calificaciones',modID:0, titulo: 'Calificaciones', url:parent.urlRoot+"/grade/report/index.php?id="+parent.idCurso});
					/*Novedades     */parent.lista.push({ id:'u0t0a0_novedades',modID:0, titulo: $('li[id=section-0] a:contains("Novedades")',data).text(), url:$('li[id=section-0] a:contains("Novedades")',data).attr('href')});
					/*Consultas     */parent.lista.push({ id:'u0t0a0_consultas',modID:0, titulo: $('li[id=section-0] a:contains("Consulta")',data).text(), url:$('li[id=section-0] a:contains("Consulta")',data).attr('href')});
					/*Presentación  */parent.lista.push({ id:'u0t0a0_presentacion',modID:0, titulo: $('li[id=section-0] a:contains("Presenta")',data).text(), url:$('li[id=section-0] a:contains("Presenta")',data).attr('href')});
					/*Cafetería     */parent.lista.push({ id:'u0t0a0_cafeteria',modID:0, titulo: $('li[id=section-0] a:contains("Cafe")',data).text(), url:$('li[id=section-0] a:contains("Cafe")',data).attr('href')});
					/*Salir         */parent.lista.push({ id:'u0t0a0_salir',modID:0, titulo: 'Salir', url:parent.urlRoot+'/my/'});
							...
							if($('.contentwithoutlink ',this).length == 0 && $('a.dimmed',this).length==0 ){//Incluir los elem solo si son elementos de actividad o recurso (no etiquetas) //REGLA 8
								...
								if($('a:contains("_noact")',this).length==0)//Evitar que una actividad especifica visible no sea tomada en cuenta
									if($('a:contains("Examen")',this).length==0 && $('a:contains("Recupera")',this).length==0 && $('a:contains("Repos")',this).length==0){
										nActividadActual++;
										parent.lista.push({ id:key, modID : RegExpPatt.exec($('a',this).attr('href'))[1], titulo: $(this).text().trim(), url:$('a',this).attr('href'),puntos:0,fInicio:0,fFinal:0});
									}else{
										parent.nExamenes++;
										parent.lista.push({ id:'u0t0a0_e'+parent.nExamenes, modID : RegExpPatt.exec($('a',this).attr('href'))[1], titulo: $(this).text().trim(), url:$('a',this).attr('href'),puntos:0,fInicio:0,fFinal:0});
									}
				    ...
				    $.get(parent.urlRoot+"/grade/report/user/index.php?id="+parent.idCurso,function(data,status){
				    	...
				    	$("th[class*='item']",data).each(function(){
				    		...
				    		$("td[headers$='"+$(this).attr("id")+" range']",data).each(function(){
                                                        ...
                                                        //console.log(idMOD+" "+idEnLista);
				    			parent.lista[idEnLista]["puntos"]=m[1];
				    		});
				    	});
                                        parent.yaCalendarizado.pop();
				    },"html");
    ...
    this.ObtenerFechas=function(data){
    	var esSoloLectura=true;
    	var RegExpPattNum=new RegExp("_([0-9]+)_");
    	var RegExpPattTipo=new RegExp("_(allowsubmissionsfromdate|duedate|cutoffdate|timeopen|timeclose|assesstimestart|assesstimefinish)");
	  ...
	  //COMENZANDO CON EL SEGUNDO CASO: Si el reporte de fechas esta en modo SOLO LECTURA
	    if(esSoloLectura){
	    	var re = /id_date_mod_(\d+)_(\w+)_(day|month|year|hour|minute)">[^<]+<\/label>(\d+|\w+)/g; 
	...
	//Ej: HTML_lista_actividades()
	 this.HTML_lista_actividades = function () {
		 ...
		 strHTML+="<tr><td>id</td><td>modID</td><td>Unidad</td><td>Tema</td><td>Actividad</td><td>Titulo</td><td>Url</td><td>Puntos</td><td>Inicio</td><td>Fin</td></tr>";
		 for(i=0;i<parent.lista.length;i++)
		 {			
			strHTML+="<tr><td>"+parent.lista[i].id+"</td><td>"+parent.lista[i].modID+"</td><td>"+parent.lista[i].id.match(/u\d+/)+"</td><td>"+parent.lista[i].id.match(/t\d+/)+"</td><td>"+parent.lista[i].id.match(/a\d+/)+"</td><td>"+parent.lista[i].titulo+"</td><td>"+parent.lista[i].url+"</td><td>"+parent.lista[i].puntos+"</td><td>"+parent.lista[i].fInicio+"</td><td>"+parent.lista[i].fFinal+"</td></tr>";
		 }
		 strHTML+="</table>";
		 return strHTML;
	 };
	...
	//Ej: GenerarCalendario()
		...
		for(i=0;i<parent.lista.length;i++){
			...
			if(i<parent.lista.length-1) nUnidadQueSigue=parseInt(String(this.lista[i+1].id.match(/u\d+/)).substring(1)); else nUnidadQueSigue=nUnidadActual+1;
			nTemaItemActual=parseInt(String(this.lista[i].id.match(/t\d+/)).substring(1));
			if(nUnidadQueSeProceso<nUnidadActual) nFilasDeUnidad=0;
			
			
			if(nUnidadActual==0){
				var nExamenActual=parseInt(String(this.lista[i].id.match(/e\d+/)).substring(1));
				if(nExamenActual>0) strHTML+="<tr class='examen'><td colspan='3' class='exa_tit'>Examen "+nExamenActual+"</td><td align='center'>"+(this.lista[i].puntos==0?"No se califica":this.lista[i].puntos)+"</td><td>"+parent.FechaCadena(this.lista[i].fInicio)+"<br>"+parent.FechaCadena(this.lista[i].fFinal)+"</td></tr>";
				continue;
			}else{
				nFilasDeUnidad++;
				...
				if(nFilasDeUnidad==1){ //Si es la primera fila de unidad
					...
					if(nTemaItemActual<nTemaAnterior && nPersistenciaTema>1){
					...
					strHTMLdeUnidad1eraLinea="Unidad "+nUnidadActual+"</td><td"+((nUnidadActual % 2 == 1)?" class='tema_impar' ":" class='tema_par' ")+" id='u"+nUnidadActual+"t"+nTemaItemActual+"'>Tema "+nTemaItemActual+"</td><td> <a href='"+this.lista[i].url+"'>"+this.lista[i].titulo+"</a></td><td align='center'>"+(this.lista[i].puntos==0?"No se califica":this.lista[i].puntos)+"</td><td>"+parent.FechaCadena(this.lista[i].fInicio)+"<br>"+parent.FechaCadena(this.lista[i].fFinal)+"</td></tr>";
				}else{//Las filas de una unidad que siguen 
					if(nTemaAnterior==nTemaItemActual){
						strHTMLdeUnidad+="<tr"+((nUnidadActual % 2 == 1)?" class='impar' ":" class='par' ")+"><td> <a href='"+this.lista[i].url+"'>"+this.lista[i].titulo+"</a></td><td align='center'>"+(this.lista[i].puntos==0?"No se califica":this.lista[i].puntos)+"</td><td>"+parent.FechaCadena(this.lista[i].fInicio)+"<br>"+parent.FechaCadena(this.lista[i].fFinal)+"</td></tr>";
						nPersistenciaTema++;
					}else{
						...
						strHTMLdeUnidad+="<tr"+((nUnidadActual % 2 == 1)?" class='impar' ":" class='par' ")+"><td"+((nUnidadActual % 2 == 1)?" class='tema_impar' ":" class='tema_par' ")+" id='u"+nUnidadActual+"t"+nTemaItemActual+"' rowspan='1'>Tema "+nTemaItemActual+"</td><td> <a href='"+this.lista[i].url+"'>"+this.lista[i].titulo+"</a></td><td align='center'>"+this.lista[i].puntos+"</td><td>"+parent.FechaCadena(this.lista[i].fInicio)+"<br>"+parent.FechaCadena(this.lista[i].fFinal)+"</td></tr>";
						nPersistenciaTema=1;
					}
	...
	this.FechaCadena=function(fecha){
		if(fecha) 
			return "<span class='dia'>"+parent.dias[fecha.getDay()]+"</span>, "+fecha.getDate()+" "+parent.meses[fecha.getMonth()]+" <span class='agno'>"+fecha.getFullYear()+"</span> - <span class='hora'>"+fecha.getHours()+":"+fecha.getMinutes()+"</span>"; 
		else return "";
	};
